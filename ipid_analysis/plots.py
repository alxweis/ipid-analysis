...
def info_strategy_distribution(
    ...
    output_path = FIGURES_DIR / Path(strategies_path).parent.name / IPID_STRATEGY_DIST_JSON_NAME
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(info, indent=2) + "\n")
    return info


def plot_strategy_distribution(strategies_path: Path, percentages: dict[str, float]) -> tuple[Figure, Path]:
    ...
    output_path = FIGURES_DIR / strategies_path.parent.name / IPID_STRATEGY_DIST_PDF_NAME
    output_path.parent.mkdir(parents=True, exist_ok=True)

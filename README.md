...
p-value after impairing ideal synthetic 4 x 25 sequences:
...
is calculated from all 80 present IP-ID values; no per-connection p-values or
subsequence minimum are used. Because this histogram-based test is
order-invariant, the two CDFs are mathematically identical. Their separate PDFs
and JSON metadata are written below
